"""
GitHub PR 実戦疎通テスト — sop_generation_workflow Phase 5 の非モック統合テスト

全フェーズを自動承認し、実際に obataka/temporal-demo へ PR を作成することを確認する。

実行手順:
    1. .env に GITHUB_TOKEN を設定（未設定の場合）:
           echo "GITHUB_TOKEN=$(gh auth token)" >> .env

    2. Docker Worker を rebuild して起動:
           docker compose up --build worker -d

    3. このスクリプトを実行（ホストマシンから）:
           python sop_github_test.py

注意:
    - GITHUB_TOKEN は docker-compose.yaml 経由で Worker コンテナに渡される
    - gh CLI と git は Dockerfile でインストール済み
    - GITHUB_TOKEN が未設定の場合、このスクリプト自身は gh CLI からの自動取得を試みるが、
      Worker コンテナ内の GITHUB_TOKEN は .env または環境変数から渡す必要がある
"""
...
BASE_BRANCH = "main"
FEATURE_BRANCH = "auto-fix/sop-first-test"
FILE_PATH = "docs/sop-integration-test.md"

TOPIC = "GitHub Activity 実戦疎通テスト用SOP（Temporal + gh CLI）"
...
async def main() -> None:
    """
    GitHub PR 実戦疎通テストのエントリポイント。

    全フェーズを自動承認し、最終的に PR URL を標準出力へ表示する。
    """
    print("=" * 60)
    print("  GitHub PR 実戦疎通テスト")
    print("=" * 60)
    ...
    workflow_id = f"sop-github-test-{uuid.uuid4().hex[:8]}"
    ...
    for phase in ("outline", "draft", "review"):
        ...
        if status["status"] == "completed":
            print("[INFO] ワークフローが予期せず完了しました（Phase 5 まで完走）")
            break
    ...
    # Phase 4（autonomous_fix）と Phase 5（github_pr）: Signal 不要なのでポーリングのみ
    print("[STEP] Phase 4（自律修正）& Phase 5（GitHub PR 作成）を待機中...")
